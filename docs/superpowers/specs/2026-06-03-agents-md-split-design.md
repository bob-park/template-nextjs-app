...
├── structure.md                   # §4 Directory + §5.8 Domain feature flow + §5.10 App Router _layouts 규칙
│                                  #   + page-scoped _components/ + route groups + special files
├── conventions/
│   ├── module-aliases.md          # §5.1
│   ├── naming.md                  # §5.2 + sub-resource API file naming
│   ├── typescript.md              # §5.3
│   └── react-sections.md          # §5.4 (Server default + Client Section Comments)
├── libs/
│   ├── zustand-slice.md           # §5.5
│   ├── ky-react-query.md          # §5.6 + Server-side fetch+cache + RQProvider defaults + HydrationBoundary
│   ├── tailwind-daisyui.md        # §5.7
│   ├── next-intl.md               # §5.9 (wiring / locale resolution / messages / usage / locale change 포함)
│   ├── theme.md                   # NEW: ThemeProvider cookie 패턴 + setTheme server action
│   └── overlay-kit.md             # NEW: overlay.open + close/unmount cleanup timing
└── workflows/
...
- `structure.md` = "어디에 두는가" 한 곳에 모음 (디렉토리 트리 + 도메인 추가 절차 + `_layouts/` 규칙 + page-scoped `_components/` + route groups + special files).
- `conventions/` = "어떻게 쓰는가" (네이밍 / TypeScript / React 섹션 주석).
- `libs/` = "어떤 라이브러리 패턴을 따르는가". `theme.md`, `overlay-kit.md` 는 자매 프로젝트 (eventify-admin-console-web) 에서 검증된 패턴을 끌어올림.
...
### 2.1 자매 프로젝트 (eventify) 에서 끌어올린 패턴

원본 AGENTS.md 에는 없지만 같은 KeyFlow BFF 계열 프로젝트인 `eventify-admin-console-web` 에서 검증되어 템플릿 표준으로 채택하는 항목:

| ID | 내용 | 반영 위치 |
|---|---|---|
| A | `_layout/` → `_layouts/` (복수) 표기 통일 | `structure.md` 전반 |
| B | Page-scoped `_components/` private folder | `structure.md` "Page sub-components" |
| C | Route Groups `(name)/` | `structure.md` "Route groups" |
| D | Special files (`loading.tsx`, `not-found.tsx`, `forbidden.tsx`) co-location | `structure.md` "Special files" |
| O | Sub-resource 별 API 파일 분리 (`sessionChairs.ts`, `sessionReviewers.ts`) | `conventions/naming.md` |
| E | Server-side fetch + React `cache()` | `libs/ky-react-query.md` |
| M+N | RQProvider defaults + `HydrationBoundary` | `libs/ky-react-query.md` |
| G | Theme `setTheme` server action 패턴 | `libs/theme.md` (신규) |
| H | overlay-kit modal cleanup timing | `libs/overlay-kit.md` (신규) |
...
4. Libs 6 파일 작성: `zustand-slice.md`, `ky-react-query.md`, `tailwind-daisyui.md`, `next-intl.md`, `theme.md`, `overlay-kit.md`.
5. Workflows 3 파일 작성: `dev-env.md`, `git.md`, `build-release.md`.
6. AGENTS.md 를 얇은 인덱스 본문으로 교체 (16개 링크).
...
- AGENTS.md 옛 §1~§8 의 내용 자체 수정 — 분리 전후 semantically identical. 표현/오타 수정만 허용.
  - 단, §2.1 표에 명시한 항목 (자매 프로젝트에서 끌어올린 패턴) 은 의도적으로 새로 추가하는 컨텐츠이며, semantic identity rule 의 예외임.
- `messages/` 실제 JSON 파일이나 `src/` 코드 변경.
- antd / `next/font/google` 패턴 추가 (이번 작업 범위 제외, 추후 별도 작업).
